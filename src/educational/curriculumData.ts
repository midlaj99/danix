...
Mixed array: ['1' '2' 'three']`,
      ...
      summary: [
        "NumPy's core data structure is numpy.ndarray, created using np.array([1, 2, 3]).",
        "NumPy stores raw, contiguous C-memory blocks in RAM, executing math 10x-50x faster than standard Python lists.",
        "NumPy arrays are strictly homogeneous: all elements share the exact same data type. Mixing numbers and strings upcasts everything to strings.",
        "Vectorization: operations like arr * 2 run in parallel across all elements at native hardware speed without slow for-loops.",
        "Standard industry convention: always import NumPy using 'import numpy as np'.",
      ],
    },
    miniPractice: {
...
3D shape: (3, 2, 2) | ndim: 3`,
      ...
      summary: [
        "1D Vectors: created from a single flat list (e.g. np.array([1, 2, 3])), representing 1 axis of data.",
        "2D Matrices: created from nested lists of lists (e.g. np.array([[1, 2], [3, 4]])), representing rows and columns.",
        "3D Tensors: created from lists of 2D matrices, representing layered grids (used in image processing and neural networks).",
        "Inspect dimensionality with arr.ndim (returns 1 for vectors, 2 for matrices, 3 for tensors).",
        "Strict shape consistency: all rows within a dimension must have identical lengths, or NumPy cannot construct a uniform array.",
      ],
    },
    miniPractice: {
...
Total memory (nbytes): 24`,
      ...
      summary: [
        "arr.ndim: returns the number of axes/dimensions (e.g., 2 for a matrix).",
        "arr.shape: returns a tuple describing array dimensions (rows, cols), such as (2, 3).",
        "arr.size: returns the total count of elements, equal to the product of shape dimensions (e.g., 2 * 3 = 6).",
        "arr.itemsize: memory footprint in bytes of a single element (e.g., 8 bytes for int64 or float64).",
        "arr.nbytes: total memory consumed by all elements in RAM, calculated as arr.size * arr.itemsize (e.g., 6 * 8 = 48 bytes).",
      ],
    },
    miniPractice: {
...
Original untouched: [1.9 2.1 3.8]`,
      ...
      summary: [
        "Core numeric types: int32, int64 (default integer), float32, float64 (default float, 8 bytes).",
        "Image and byte data: uint8 stores unsigned integers from 0 to 255 (the standard format for RGB pixels).",
        "Boolean type: bool_ stores True or False, occupying 1 byte per element.",
        "Explicit type conversion: use arr.astype(np.float64) or arr.astype('int32') — astype always creates an independent copy.",
        "Automatic type inference: NumPy automatically assigns the smallest safe type or upcasts mixed numeric values (e.g., int + float -> float64).",
      ],
    },
    miniPractice: {
...
Mutated matrix:
 ...
 [40 50 60]]`,
      ...
      summary: [
        "Zero-based indexing: first item is at index 0, second at index 1, and the last item at index -1.",
        "2D Indexing: use comma notation arr[row, col] (e.g., arr[0, 1]) instead of chained arr[0][1] for faster C-speed access.",
        "Negative indexing: arr[-1] selects the last row; arr[:, -1] selects the last column.",
        "Index safety: accessing an index outside array bounds immediately raises an IndexError.",
        "In-place assignment: assigning to an indexed slot (arr[0, 1] = 99) directly mutates the original array in memory.",
      ],
    },
    miniPractice: {
...
Reversed rows:
 ...
 [ 1  2  3  4]]`,
      ...
      summary: [
        "Slicing syntax: arr[start : stop : step] — start is inclusive, stop is exclusive.",
        "2D submatrix slicing: arr[row_start:row_stop, col_start:col_stop] cuts out rectangular regions.",
        "Full-axis slicing: a lone colon ':' selects all elements along that axis (e.g., arr[:, 0] grabs column 0).",
        "Step intervals & reversal: arr[::2] selects every second element; arr[::-1] reverses elements along that axis.",
        "Critical Rule: basic slicing produces a memory VIEW, not a copy — modifying a slice alters the original array unless you call .copy()!",
      ],
    },
    miniPractice: {
...
Auto rows shape: (3, 2)`,
      ...
      summary: [
        "arr.reshape(rows, cols): restructures elements into new dimensions without modifying the underlying data buffer.",
        "Conservation of elements: the total element count must stay identical (new_rows * new_cols == arr.size).",
        "Automatic inference (-1): pass -1 for one dimension (e.g., arr.reshape(2, -1)) and NumPy automatically calculates the required size.",
        "arr.flatten(): flattens any multi-dimensional array into a 1D vector and ALWAYS returns an independent deep copy.",
        "arr.ravel(): flattens into 1D as a memory VIEW whenever possible, avoiding memory allocation overhead.",
      ],
    },
    miniPractice: {
...
Identity Matrix:
 ...
 [0. 0. 1.]]`,
      ...
      summary: [
        "np.zeros(shape): creates an array filled with 0.0 (default dtype is float64).",
        "np.ones(shape): creates an array filled with 1.0 (default dtype is float64).",
        "np.full(shape, fill_value): creates an array filled entirely with fill_value (e.g., np.full((2, 2), 7)).",
        "np.eye(N): creates an N×N square Identity Matrix with 1.0 on the main diagonal and 0.0 everywhere else.",
        "Shape parameter: always provide multi-dimensional shapes as a tuple, e.g., np.zeros((3, 4)) rather than np.zeros(3, 4).",
      ],
    },
    miniPractice: {
...
linspace(0, 1, 5): [0.   0.25 0.5  0.75 1.  ]`,
      ...
      summary: [
        "np.arange(start, stop, step): generates values based on an explicit step interval (stop value is exclusive).",
        "np.linspace(start, stop, num): generates num evenly spaced points across an interval (stop value is INCLUSIVE by default).",
        "When to use arange: when you know the exact spacing interval between numbers (e.g. counting by 2s).",
        "When to use linspace: when you know the exact count of sample points needed (e.g. 50 points for plotting graphs).",
        "Floating-point safety: np.linspace avoids cumulative floating-point rounding errors that can occur with fractional steps in np.arange.",
      ],
    },
    miniPractice: {
...
Modulo (a % 4): [2 0 2]`,
      ...
      summary: [
        "Vectorized arithmetic: operators +, -, *, /, // (floor div), % (modulo), ** (power) act element-wise across arrays.",
        "Hardware parallelism: operations execute simultaneously at C speed via SIMD instructions without Python for-loops.",
        "In-place arithmetic: +=, -=, *= mutate the existing array buffer directly, saving memory allocation time.",
        "Division by zero: produces inf or nan with a runtime warning rather than crashing your program.",
        "Shape compatibility: arrays must have identical shapes or conform to broadcasting rules to perform arithmetic.",
      ],
    },
    miniPractice: {
...
Rounded values: [1. 3. 4.]`,
      ...
      summary: [
        "Mathematical ufuncs: np.sqrt(), np.abs(), np.exp(), np.log() (natural log ln), and np.log10() (base-10 log).",
        "Rounding ufuncs: np.round(arr, decimals), np.floor() (round down), and np.ceil() (round up).",
        "Trigonometry: np.sin(), np.cos(), np.tan() evaluate angles in radians (convert degrees with np.radians(deg)).",
        "Performance power: Universal Functions (ufuncs) execute pre-compiled C-loops over millions of numbers in milliseconds.",
        "UFunc broadcasting: ufuncs automatically apply broadcasting rules across multi-dimensional inputs.",
      ],
    },
    miniPractice: {
...
Standard Deviation: 14.14`,
      ...
      summary: [
        "Core aggregations: np.sum(), np.min(), np.max(), np.mean(), np.median(), and np.std() (standard deviation).",
        "Variance: np.var() computes statistical variance, which is the square of standard deviation (std = sqrt(var)).",
        "Extremum positions: np.argmin(arr) and np.argmax(arr) return the flat index position of the minimum and maximum values.",
        "Dual syntax: most aggregations can be called as array methods (arr.sum()) or module functions (np.sum(arr)).",
        "Global reduction: calling an aggregation without an axis parameter collapses the entire array into a single scalar value.",
      ],
    },
    miniPractice: {
...
(5, 8) mean(axis=0) shape: (8,)`,
      ...
      summary: [
        "axis=0: operates vertically DOWN columns, collapsing rows (output length equals the number of columns).",
        "axis=1: operates horizontally ACROSS rows, collapsing columns (output length equals the number of rows).",
        "2D Matrix Extremums: arr.max(axis=0) finds the peak of each column; arr.max(axis=1) finds the peak of each row.",
        "Dimensional collapse: aggregating across an axis reduces ndim by 1 (e.g. shape (3, 4) with axis=0 becomes (4,)).",
        "keepdims=True: preserves the collapsed axis as length 1 (shape (3, 1)), keeping the result ready for broadcast operations.",
      ],
    },
    miniPractice: {
...
arr == arr (element-wise): [ True  True  True  True]`,
      ...
      summary: [
        "Vectorized comparisons: >, <, >=, <=, ==, != compare every element and return an array of boolean flags (True/False).",
        "np.all(mask): returns True only if EVERY single element in the boolean array evaluates to True (logical AND).",
        "np.any(mask): returns True if AT LEAST ONE element in the boolean array evaluates to True (logical OR).",
        "np.array_equal(a, b): checks if two arrays have the exact same shape and identical element values.",
        "Python vs NumPy: always use np.array_equal() or (a == b).all() instead of Python's 'a == b', which returns an array of booleans.",
      ],
    },
    miniPractice: {
...
2D filtered positives: [2 3] shape: (2,)`,
      ...
      summary: [
        "Boolean masking: arr[mask] filters and extracts only the elements where the mask condition is True.",
        "1D Flattening rule: boolean indexing on a 2D or 3D array always returns a flat 1D array of matching values.",
        "Bitwise logical operators: use & (AND), | (OR), and ~ (NOT) instead of Python's 'and', 'or', 'not' keywords.",
        "Parenthesis requirement: always enclose individual conditions in parentheses, e.g., (arr > 10) & (arr < 50).",
        "Conditional assignment: arr[arr < 0] = 0 replaces all negative numbers in-place in a single line.",
      ],
    },
    miniPractice: {
...
Failing student indices: [0 3]`,
      ...
      summary: [
        "Ternary selection: np.where(condition, x, y) returns elements from x where condition is True, and from y where False.",
        "Shape preservation: np.where(condition, x, y) preserves the exact original shape and dimensionality of the inputs.",
        "Coordinate search: calling np.where(condition) with only the condition returns a tuple of index coordinate arrays.",
        "Vectorized branch: completely replaces Python if-else loops and list comprehensions with native C speed.",
        "np.argwhere(condition): returns matching indices organized as coordinate pairs [[row, col], ...].",
      ],
    },
    miniPractice: {
...
Passing Average: 79.4`,
      ...
      summary: [
        "Condition counting: np.sum(arr > threshold) or (arr > threshold).sum() counts how many elements satisfy a condition.",
        "Percentage calculation: np.mean(arr > threshold) * 100 calculates the exact percentage of elements meeting a condition.",
        "Filtered statistics: arr[arr > 0].mean() calculates the average exclusively across positive values.",
        "Value clamping: np.clip(arr, min_val, max_val) caps values below min_val and above max_val to the boundary values.",
        "Data cleaning pipelines: combine boolean masking, replacement, and clipping to sanitize real-world datasets.",
      ],
    },
    miniPractice: {
...
Broadcasted sum:
 ...
 [14 25 36]]`,
      ...
      summary: [
        "The Broadcasting concept: allows arithmetic operations between arrays of different shapes without copying data.",
        "Scalar broadcasting: a single scalar expands virtually across an entire array of any shape (e.g. matrix + 10).",
        "Zero memory waste: broadcasting creates virtual dimension stretches at the CPU hardware level with zero extra RAM allocated.",
        "Alignment rule: NumPy compares array shapes starting from the trailing (rightmost) dimension and moves backward.",
        "Compatibility test: two dimensions are compatible if they are equal, or if one of them is 1.",
      ],
    },
    miniPractice: {
...
# Trailing dimensions 3 and 2 do not match and neither is 1 -> ValueError!`,
      ...
      summary: [
        "Broadcasting Rule 1: If arrays have different numbers of dimensions, pad the smaller shape with 1s on the left.",
        "Broadcasting Rule 2: Dimensions are compatible if they have equal lengths, or if one of them has length 1.",
        "Dimension stretching: any axis with length 1 is virtually repeated to match the length of the larger axis.",
        "Incompatible shapes: if dimensions disagree and neither is 1, NumPy raises ValueError: operands could not be broadcast together.",
        "Example match: shape (4, 3) and shape (3,) pads to (1, 3), which stretches to (4, 3) — fully compatible!",
      ],
    },
    miniPractice: {
...
Broadcasted 3D sum shape: (10, 5, 4)`,
      ...
      summary: [
        "Outer product broadcasting: a column vector (N, 1) and row vector (1, M) broadcast together into a full (N, M) matrix.",
        "Column-wise centering: arr - arr.mean(axis=0) subtracts each column's average from all rows simultaneously.",
        "Row-wise centering: arr - arr.mean(axis=1, keepdims=True) maintains shape (N, 1) to subtract row averages correctly.",
        "Coordinate grids: np.meshgrid(x, y) builds 2D coordinate matrices for evaluating 2D spatial functions and surfaces.",
        "Loop elimination: broadcasting replaces nested loops in distance matrix calculations, heatmaps, and image filters.",
      ],
    },
    miniPractice: {
...
np.arange concat length: 7`,
      ...
      summary: [
        "np.concatenate([a, b], axis=0): joins multiple arrays along an EXISTING axis (default is axis=0).",
        "Axis 0 concatenation: joins arrays vertically (stacking rows), requiring identical column counts.",
        "Axis 1 concatenation: joins arrays horizontally (stacking columns), requiring identical row counts.",
        "Sequence argument: arrays to concatenate must be passed inside a list or tuple: np.concatenate([a, b]).",
        "Existing axis rule: concatenate does not create new dimensions; input arrays must already share matching ndim.",
      ],
    },
    miniPractice: {
...
stack total size: 9 | ndim: 2`,
      ...
      summary: [
        "np.stack([a, b], axis=...): joins arrays along a BRAND NEW axis, increasing the total ndim by 1.",
        "Strict shape match: all arrays passed to np.stack must have exactly identical shapes.",
        "1D to 2D stacking: stacking two 1D vectors of length 3 along axis=0 produces shape (2, 3); along axis=1 produces (3, 2).",
        "Stack vs Concatenate: concatenate extends an existing axis; stack creates a higher-dimensional tensor.",
        "Practical application: stacking individual 2D image matrices into a 3D batch or video sequence.",
      ],
    },
    miniPractice: {
...
2D hstack (3, 2) + (3, 5) shape: (3, 7)`,
      ...
      summary: [
        "np.vstack([a, b]): vertical stacking helper (equivalent to concatenating along axis=0 for 2D arrays).",
        "np.hstack([a, b]): horizontal stacking helper (equivalent to concatenating along axis=1 for 2D arrays).",
        "1D vector behavior: vstack turns 1D vectors into rows of a 2D matrix; hstack joins 1D vectors into a longer 1D vector.",
        "np.dstack([a, b]): depth stacking helper that combines arrays along the 3rd axis (used for RGB color layers).",
        "Readability advantage: convenience helpers make code concise and self-documenting without explicit axis arguments.",
      ],
    },
    miniPractice: {
...
axis=1 split sub-array shape: (6, 2)`,
      ...
      summary: [
        "np.split(arr, sections, axis=0): partitions an array into multiple sub-arrays along the specified axis.",
        "Strict equal division: np.split(arr, N) requires the axis length to be evenly divisible by N (raises ValueError otherwise).",
        "np.array_split(arr, N): safely divides an array into N sections even when the length is not evenly divisible.",
        "Split index list: passing a list of indices like [2, 5] splits the array before index 2 and before index 5.",
        "Directional helpers: np.vsplit() splits vertically (rows); np.hsplit() splits horizontally (columns).",
      ],
    },
    miniPractice: {
...
Ranked players: ['Goblin' 'Hero' 'Ronin' 'Aria']`,
      ...
      summary: [
        "np.sort(arr, axis=-1): returns a sorted copy of the array along the specified axis without mutating original data.",
        "In-place sorting: arr.sort() sorts the array directly in place in memory and returns None.",
        "np.argsort(arr): returns the integer indices that would sort the array, enabling indirect sorting and ranking.",
        "Extremum location: np.argmax(arr) and np.argmin(arr) return the indices of maximum and minimum values.",
        "Multi-axis sorting: sorting along axis=0 sorts each column independently; axis=1 sorts each row independently.",
      ],
    },
    miniPractice: {
...
Original after copy mutation: [99  2  3  4]`,
      ...
      summary: [
        "Array View: shares the same underlying memory buffer with another array; modifying a view mutates the original data!",
        "Basic slicing creates views: sub = arr[0:2] is a view — modifying sub[0] directly changes arr[0].",
        "Array Copy: arr.copy() creates an independent memory buffer; changes to the copy never affect the original array.",
        "View verification: check arr.base — if arr.base is None, it owns its memory; if it references an array, it is a view.",
        "Safety guideline: always call .copy() when you need to isolate, modify, or sanitize a subset of data.",
      ],
    },
    miniPractice: {
...
Transposed shape: (3, 2)`,
      ...
      summary: [
        "Matrix transpose: arr.T swaps rows and columns (swaps axes 0 and 1).",
        "Double transpose identity: (arr.T).T restores the original shape and layout identically.",
        "1D vector behavior: transposing a 1D vector arr.T has no effect and remains 1D because it only has 1 axis.",
        "np.transpose(arr, axes=(...)): permutes arbitrary dimension orders for 3D and higher-dimensional tensors.",
        "Zero-copy operation: transposition simply adjusts the shape and strides metadata without copying data in RAM.",
      ],
    },
    miniPractice: {
...
ravel() shares memory with mat? True`,
      ...
      summary: [
        "np.squeeze(arr): removes all single-dimensional entries (dimensions of length 1) from the array shape.",
        "Targeted squeeze: np.squeeze(arr, axis=1) removes only the specified length-1 axis while preserving others.",
        "arr.flatten(): collapses any array into a 1D vector and ALWAYS creates an independent memory copy.",
        "arr.ravel(): collapses any array into a 1D vector as a VIEW whenever possible, maximizing speed and memory efficiency.",
        "Dimension stripping: essential for removing unnecessary batch or channel dimensions after model predictions.",
      ],
    },
    miniPractice: {
...
Gaussian array shape: (4, 5)`,
      ...
      summary: [
        "Modern Random API: always instantiate the generator with rng = np.random.default_rng(seed).",
        "Seed reproducibility: setting a fixed integer seed (e.g. 42) ensures 100% identical random sequences on every run.",
        "Random integers: rng.integers(low, high, size) generates uniform random integers (high bound is exclusive).",
        "Uniform floats: rng.random(size) generates floating-point values uniformly distributed in [0.0, 1.0).",
        "Gaussian distribution: rng.normal(loc, scale, size) draws samples from a normal distribution with mean 'loc' and std 'scale'.",
      ],
    },
    miniPractice: {
...
print("Probability of rolling 7:", np.round(prob_seven, 3))`,
      ...
      summary: [
        "In-place shuffling: rng.shuffle(arr) randomly shuffles the elements of an array in place and returns None.",
        "Permutation copy: rng.permutation(arr) returns a randomly shuffled new copy without modifying the original array.",
        "Random sampling: rng.choice(arr, size, replace=True/False) draws random elements with or without replacement.",
        "Weighted sampling: pass probability weights p=[...] to rng.choice to sample elements with custom likelihoods.",
        "Monte Carlo simulation: uses repeated random sampling to estimate probabilities, game outcomes, and statistical metrics.",
      ],
    },
    miniPractice: {
...
(2, 3) @ (3, 4) result shape: (2, 4)`,
      ...
      summary: [
        "Element-wise multiplication: A * B multiplies matching elements (requires identical or broadcastable shapes).",
        "Matrix multiplication: A @ B or np.matmul(A, B) performs standard linear algebra row-by-column matrix multiplication.",
        "Dimension rule: for A @ B with shapes (M, K) and (K, N), inner dimensions must match (K == K), yielding shape (M, N).",
        "Non-commutative property: matrix multiplication is generally order-dependent (A @ B != B @ A).",
        "Vector-matrix products: multiplying a 1D vector of length K by a matrix (K, N) yields a 1D vector of length N.",
      ],
    },
    miniPractice: {
...
Outer product shape (2, 3): (2, 3)`,
      ...
      summary: [
        "np.dot(a, b): calculates the sum of products sum(a[i] * b[i]) for two 1D vectors.",
        "Geometric meaning: dot(a, b) = |a| * |b| * cos(theta), measuring the directional alignment of two vectors.",
        "Orthogonal vectors: perpendicular vectors have a dot product of exactly 0.0.",
        "Outer product: np.outer(a, b) generates an (M, N) matrix where entry (i, j) is a[i] * b[j].",
        "2D matrix dot: for 2D arrays, np.dot(A, B) performs matrix multiplication equivalent to A @ B.",
      ],
    },
    miniPractice: {
...
det(np.eye(4)): 1.0`,
      ...
      summary: [
        "Linear algebra submodule: advanced LAPACK operations are accessed via the np.linalg module.",
        "Solving systems: np.linalg.solve(A, b) solves the linear system Ax = b faster and with greater numerical precision than inv(A) @ b.",
        "Determinant: np.linalg.det(A) computes the scaling factor; det != 0 confirms a matrix is non-singular and invertible.",
        "Matrix inverse: np.linalg.inv(A) computes A^-1 such that A @ A^-1 equals the identity matrix (requires square matrix).",
        "Vector & Matrix Norm: np.linalg.norm(x) calculates Euclidean length/magnitude (square root of sum of squares).",
      ],
    },
    miniPractice: {
...
nanmax(): 26.0`,
      ...
      summary: [
        "What is NaN: np.nan represents undefined or unrepresentable floating-point values (e.g., 0.0 / 0.0).",
        "IEEE 754 property: np.nan != np.nan evaluates to True — never check for NaN using equality 'arr == np.nan'!",
        "NaN detection: always use np.isnan(arr) to generate a boolean mask of NaN positions.",
        "NaN contamination: standard functions like np.sum() and np.mean() return nan if even one NaN is present.",
        "NaN-safe aggregations: use np.nansum(), np.nanmean(), np.nanmin(), np.nanmax(), and np.nanstd() to calculate while ignoring NaNs.",
      ],
    },
    miniPractice: {
...
print("Imputed clean dataset:", data)`,
      ...
      summary: [
        "Counting missing values: np.isnan(arr).sum() counts the total number of missing NaN entries.",
        "Filtering out NaNs: arr[~np.isnan(arr)] extracts all valid, non-missing values into a clean 1D array.",
        "Mean imputation: arr[np.isnan(arr)] = np.nanmean(arr) replaces all missing values with the average of valid elements.",
        "Value substitution: np.nan_to_num(arr, nan=0.0) cleanly replaces NaNs with 0.0 or any designated constant.",
        "Infinite values: np.isinf(arr) detects positive/negative infinity, which can also be sanitized with np.nan_to_num().",
      ],
    },
    miniPractice: {
...
Peak Temperature Recorded: 32.1`,
      ...
      summary: [
        "Z-score Standardization: (X - X.mean(axis=0)) / X.std(axis=0) centers features to mean 0 and standard deviation 1.",
        "Min-Max Normalization: (X - X.min(axis=0)) / (X.max(axis=0) - X.min(axis=0)) rescales data into the [0.0, 1.0] range.",
        "Percentiles: np.percentile(arr, q, axis=0) computes the q-th percentile (q=50 is median, IQR is q=75 - q=25).",
        "Data orientation: tabular datasets arrange samples as rows (axis=0) and individual features as columns (axis=1).",
        "Preprocessing importance: standardizing features prevents high-magnitude features from dominating machine learning models.",
      ],
    },
    miniPractice: {
...
print("Calculated 1,000,000 numbers instantly! First 3:", result[:3])`,
      ...
      summary: [
        "Vectorization concept: replaces explicit Python for-loops with batch operations executed in pre-compiled C.",
        "SIMD hardware: Single Instruction, Multiple Data registers process 4 to 8 numbers in a single CPU clock cycle.",
        "CPU Cache locality: storing numbers in contiguous memory ensures maximum L1/L2 cache hits, eliminating memory wait times.",
        "np.vectorize(func): convenience wrapper that allows custom Python functions to accept array arguments with vectorized syntax.",
        "Performance hierarchy: native NumPy ufuncs run at pure C speed and are substantially faster than np.vectorize wrappers.",
      ],
    },
    miniPractice: {
...
Total bytes (nbytes = size * itemsize): 8000`,
      ...
      summary: [
        "Python list memory: stores an array of memory pointers referencing scattered PyObject wrappers on the heap.",
        "NumPy array memory: stores pure raw bytes sequentially in a single contiguous block in RAM.",
        "Memory compactness: NumPy arrays consume 4x to 8x less RAM than equivalent Python lists.",
        "Memory inspection: arr.itemsize reveals bytes per item (8 for int64), and arr.nbytes gives the total memory footprint.",
        "Zero pointer overhead: direct memory addressing eliminates pointer chasing and type checking on every iteration.",
      ],
    },
    miniPractice: {
...
Original untouched: 50`,
      ...
      summary: [
        "Fancy Indexing: passing lists or arrays of integers (e.g., arr[[0, 2, 4]]) extracts elements at those specific indices.",
        "Output shape: the shape of the result reflects the shape of the index arrays, not the shape of the original array.",
        "Multi-dimensional indexing: arr[[r1, r2], [c1, c2]] pairs row and column coordinates to extract specific points.",
        "Always creates a COPY: unlike simple slicing which produces views, fancy indexing ALWAYS allocates an independent copy!",
        "Row/Column reordering: fancy indexing is the standard technique for shuffling, gathering, or reordering dataset rows.",
      ],
    },
    miniPractice: {
...
atleast_3d shape: (1, 2, 1)`,
      ...
      summary: [
        "np.newaxis: inserts a new axis of length 1 at the specified location (e.g., arr[:, np.newaxis] turns (N,) into (N, 1)).",
        "np.expand_dims(arr, axis): functional method to insert a length-1 axis at any desired dimension index.",
        "Helper functions: np.atleast_2d(arr) ensures at least 2 dimensions; np.atleast_3d(arr) ensures at least 3 dimensions.",
        "Column vector transformation: converting 1D vectors to 2D column vectors (N, 1) is essential for matrix math and broadcasting.",
        "Deep learning tensor preparation: dimension expansion is used to add batch and channel axes before neural network inference.",
      ],
    },
    miniPractice: {
...
Cumulative sum: [ 1  3  6 10]`,
      ...
      summary: [
        "np.clip(arr, min_val, max_val): clamps array values to remain strictly within specified lower and upper bounds.",
        "np.unique(arr, return_counts=True): finds sorted unique values and optionally returns their frequency counts.",
        "np.cumsum(arr, axis=...): computes the running cumulative sum along an axis (essential for time-series analysis).",
        "np.diff(arr, axis=...): computes the discrete difference between adjacent elements: arr[i+1] - arr[i].",
        "np.interp(x_new, x_known, y_known): performs 1D linear interpolation for query points based on sample coordinates.",
      ],
    },
    miniPractice: {
...
Identity np.eye(4) determinant: 1.0`,
      ...
      summary: [
        "Full Mastery Synthesis: seamlessly combines slicing, reshaping, broadcasting, vectorization, and linear algebra.",
        "Production Data Pipeline: Clean missing data (NaNs) -> Standardize features (Z-score) -> Reshape/Transform -> Aggregate.",
        "Memory Architecture: instinctively recognize views (slicing, ravel, transpose) vs independent copies (fancy indexing, flatten, astype).",
        "Multi-Axis Intuition: collapse rows with axis=0 (column-wise reduction) and collapse columns with axis=1 (row-wise reduction).",
        "Pure Vectorized Mindset: write high-performance, elegant numerical code by eliminating Python loops entirely.",
      ],
    },
    miniPractice: {
